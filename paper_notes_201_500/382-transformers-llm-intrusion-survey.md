# 382. Transformers and Large Language Models for Efficient Intrusion Detection Systems: A Comprehensive Survey
...
## Method / Framework
作者通过文献检索、筛选和 bibliometric analysis 建立综述流程，按模型与应用构建 taxonomy：

- attention-based Transformer；
- CNN/LSTM-Transformer hybrid；
- Vision Transformer；
- GAN-Transformer；
- GPT/decoder-only 和 BERT/encoder-only 模型；
- 网络、主机、IoT、SDN、工业控制和关键基础设施场景。
...
公平比较的难点是不同论文使用不同网络环境、攻击标签、类别平衡和数据切分；因此单看某篇的 accuracy 很容易把数据泄漏或预处理收益误认为架构收益。
...
作者强调不同攻击类型和环境需要不同数据表示：网络包/流量的数值表格并不天然等同于自然语言 token，直接把 LLM 用在 IDS 上不能自动获得语义理解。数据不平衡、未知攻击、加密流量、资源受限设备和 adversarial evasion 是贯穿各研究的共同困难。
...
解释性错误包括 attention 被误读为因果重要性、模型只记住 dataset artifacts、异常标签本身不可靠，以及自然语言 LLM 生成的“攻击原因”可能是事后合理化。安全场景还需要对抗测试，验证攻击者改变非关键字段时模型和解释是否稳定。

## Limitations
survey 的论文选择和 bibliometric 统计依赖检索词、年份和作者筛选，且不同研究缺少统一 protocol。LLM 相关 IDS 仍处于早期，很多结果来自小数据集或离线实验；大模型的推理成本、隐私、实时性和可解释性尚未在生产环境充分验证。本文的可解释性讨论是研究议程而非已验证解决方案。
